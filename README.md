...
We can check whether the clusters obtained by using DMCTs for different cell types are different. We can look at the pre-generated heatmap `clustHeatmap` displaying the standardized residual matrix annotated by the clustering assignments obtained by using DMCTs for different cell types: 
```{r chunk6,message=F}
